...
public class CommentDao extends AbstractMFlixDao {
    ...
    public boolean updateComment(String commentId, String text, String email) {
        ...
        Document docText = new Document("text", text);
        comment = commentCollection.find(queryFilter).first();
        if (comment != null) {
            UpdateResult updateResult = commentCollection.updateOne(queryFilter, new Document("$set", docText));
            // TODO> Ticket - Handling Errors: Implement a try catch block to
            // handle a potential write exception when given a wrong commentId.
    ...
    public List<Critic> mostActiveCommenters() {
        ...
        List<Bson> pipeline = Arrays.asList(
                new Document("$sortByCount", "$email"),
